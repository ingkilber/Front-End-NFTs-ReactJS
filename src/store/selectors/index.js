...
export const nftAll = (state) => state.NFT.nfts;
export const nftShowcaseState = (state) => state.NFT.nftShowcase;
export const nftDetailState = (state) => state.NFT.nftDetail;
export const nftUserState = (state) => state.NFT.nftUser;
export const blockchainConnectionState = (state) => state.BLOCKCHAIN.blockchainConnection;
export const userState = (state) => state.USER.get;
...
export const allNftItems = createSelector(nftFilter, nftAll, ( filters, nfts ) => {
    let { data } = nfts;
    ...
    if(categories.size) {
        data = data.filter( nft => {
            const catIds = nft.categories.map(category => category.id)
            const [category] = categories
            return catIds.includes(category)
        });
    }
    if(status.size) {
        data = data.filter( nft => status.has(nft.statusId));
    }
    if(itemsType.size) {
    ...
    if(nftTitle.trim().length) {
        ...
        console.log(pattern)
        data = data.filter( nft => nft.title_es.match(pattern) || nft.title_en.match(pattern));
    }
...
export const nftUserItems = createSelector(nftFilter, nftUserState, ( filters, nfts ) => {
    let { data } = nfts;
    const { categories, status, itemsType, collections, nftTitle } = filters;
